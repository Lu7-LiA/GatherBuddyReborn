...
2026-09-16時点では、3地点間の最大3次元距離が小さい順に各属性の上位3 Baseを選択した。候補が2件だけのFireはBase 914と911を採用した。両Baseはオミクロンのクエスト用だが、実際の採集Territoryは高地ドラヴァニア。Earthは候補なし。
...
合計14 Base・42 Point。既存のBase 542はLightning候補内で9位だったため置き換えた。
